...
def main(epoch, start_Ts):
    ...
    for data_idx, datum in enumerate(dataloaders["valid"]):
        ...
        # filter out molecules that have rings, too much branching,
        # double bonds, or charge
        ...
        gs_e, gs_f = get_ef(gs, num_threads=16)

        for start_T in start_Ts:
            print("starting", start_T, "...")
            # Make out_dir
            out_dir = "outputs/qm9_mc/flexible_mols/diffusion/T{:0>2d}/{:0>4d}"
            out_dir = out_dir.format(start_T, data_idx)
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)

            # save gs structure & energy to out_dir
            ase.io.write(os.path.join(out_dir, "gs.xyz"), gs)
            with open(os.path.join(out_dir, "gs.energy"), "w") as f:
                f.write(str(gs_e))


            # run the diffusion MC
            start_time = time.time()
            xyz_chain, charges_chain = run_dmc(args, model, dataset_info, start_T,
                                               xyz, one_hot, atomic_nums, charges)
            print("DMC time:", time.time() - start_time)

            # save results to out_dir/chain.npz
            results = {"atomic_nums": atomic_nums.numpy(),
                       "xyz_chain": np.stack(xyz_chain),
                       "charges_chain": np.stack(charges_chain)}
            out_fn = os.path.join(out_dir, "chain.npz")
            np.savez(out_fn, **results)


def run_dmc(args, model, dataset_info, start_T, xyz, one_hot, atomic_nums, charges):
    ...
    for mc_step in range(N_MC_STEPS):
        ...
        diffused_one_hot, diffused_charges, diffused_x, diffused_node_mask, chain = \
            sample(args, args.device, model, dataset_info,
                   ...
                   start_T=start_T, end_T=start_T - 1)
        ...
        all_charges.append((charges[0] - atomic_nums).numpy())
    ...
    return all_xyz, all_charges


if __name__ == "__main__":
    start_Ts = [40, 50, 60, 70, 80, 90, 100, 125, 150]
    epoch = 5150
    main(5150, start_Ts)
