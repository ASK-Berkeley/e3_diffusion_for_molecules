...
def main(epoch):
    ...
    for data_idx, datum in enumerate(dataloaders["valid"]):
        ...
        numbers = [[{"H": 1, "C": 6, "N": 7, "O": 8, "F": 9}[dataset_info["atom_decoder"][a]]
                    for a in chain[i,:,3:8].float().argmax(dim=1)]
                   for i in range(chain.shape[0])]
        numbers = torch.tensor(numbers).cpu()
        charges = chain[:,:,8].round()
        mask = diffused_node_mask[:,:,0].cpu().view(-1) == 1
        es = []
        for i in range(0, numbers.shape[0], 2):
            e, f = get_ef(Atoms(numbers=numbers[i,mask], positions=chain[i,mask,:3].cpu()))
            es.append(e - gs_e)
            #print(e - gs_e)
        print(es)
